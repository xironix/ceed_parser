...
static void test_bitcoin_wallet_generation(void) {
    const char *seed_phrase = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";
    Wallet wallet;
    
    TEST_ASSERT(wallet_generate_from_seed(seed_phrase, WALLET_TYPE_BITCOIN, NULL, &wallet));
...
static void test_ethereum_wallet_generation(void) {
    ...
    TEST_ASSERT(wallet_generate_from_seed(seed_phrase, WALLET_TYPE_ETHEREUM, NULL, &wallet));
...
static void test_monero_wallet_generation(void) {
    const char *seed_phrase = "gels zeal lucky jeers irony tamper older pests noggin orange android academy bailed mural tossed accent atlas layout drinks ozone academy academy avatar onset";
    Wallet wallet;
    
    TEST_ASSERT(wallet_generate_from_seed(seed_phrase, WALLET_TYPE_MONERO, NULL, &wallet));
...
void run_wallet_tests(void) {
    ...
    test_bitcoin_wallet_generation();
    test_ethereum_wallet_generation();
    test_monero_wallet_generation();
} 