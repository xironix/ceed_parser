...
void simd_detect_features(simd_features_t* features) {
    ...
    // For example, on Linux: /sys/devices/system/cpu/cpu0/cache/index0/coherency_line_size
#endif
}
...
bool simd_available(void) {
    ...
    return features.has_avx || features.has_avx2 || 
           features.has_sse4_1 || features.has_sse4_2 || 
           features.has_neon;
}
...
const char* simd_strstr(const char* haystack, const char* needle) {
    ...
    while (strlen(haystack) >= 32) {
        ...
        __m256i match = _mm256_and_si256(eq_first, 
                      _mm256_and_si256(eq_second,
                      _mm256_and_si256(eq_third, eq_fourth)));
    ...
    for (size_t i = 0; i <= len - needle_len; i += 16) {
        ...
        int idx = _mm_cmpestri(_mm_loadu_si128((__m128i*)(needle)), needle_len,
                             _mm_loadu_si128((__m128i*)(haystack + i)), block_len,
                             mode);
...
int simd_strcmp(const char* str1, const char* str2) {
    ...
    while (1) {
        ...
        uint64x2_t eq_merged = vpaddq_u64(vreinterpretq_u64_u8(eq), vreinterpretq_u64_u8(eq));
        ...
        // Check for null terminator
        uint64x2_t zero_merged = vpaddq_u64(vreinterpretq_u64_u8(has_zero), vreinterpretq_u64_u8(has_zero));
...
static uint32_t bloom_hash3(const char* str) {
...
bool bloom_filter_init(bloom_filter_t* filter, size_t expected_items, double false_positive_rate) {
    ...
    size_t size = (size_t)(-1.0 * expected_items * log(false_positive_rate) / (log(2) * log(2)));
    size_t hash_functions = (size_t)(size / expected_items * log(2));
