...
bool memory_pool_init(memory_pool_t* pool, size_t block_size, size_t max_blocks, size_t small_capacity) {
    ...
    pool->small_capacity = small_capacity > 0 ? small_capacity : DEFAULT_SMALL_CAPACITY;
    
    // Allocate small blocks array
    pool->small_blocks = (small_block_t*)calloc(pool->small_capacity, sizeof(small_block_t));
    if (!pool->small_blocks) {
        return false;
    }
    
    // Allocate first block
    pool->blocks = (memory_block_t*)malloc(sizeof(memory_block_t) + pool->block_size);
    if (!pool->blocks) {
 ...
 */
static memory_block_t* memory_pool_allocate_block(memory_pool_t* pool, size_t min_size) {
    ...
    // Allocate new block
    memory_block_t* block = (memory_block_t*)malloc(sizeof(memory_block_t) + block_size);
    if (!block) {
        return NULL;
    }
...
void* memory_pool_alloc(memory_pool_t* pool, size_t size) {
    ...
    if (!pool->current_block || pool->current_block->used + size > pool->current_block->size) {
...
static void tls_pool_init_once(void) {
    ...
    if (!memory_pool_init(g_global_pool, DEFAULT_BLOCK_SIZE, DEFAULT_MAX_BLOCKS, DEFAULT_SMALL_CAPACITY)) {
        free(g_global_pool);
        g_global_pool = NULL;
    ...
    if (pthread_key_create(&g_global_pool->tls_key, tls_pool_thread_cleanup) != 0) {
...
void tls_pool_destroy(void) {
    ...
    if (g_global_pool) {
        // Don't destroy the TLS key; let the thread cleanup functions handle their pools
        memory_pool_destroy(g_global_pool);
        free(g_global_pool);
        g_global_pool = NULL;
    }
...
static memory_pool_t* get_thread_pool(void) {
    ...
    memory_pool_t* pool = (memory_pool_t*)pthread_getspecific(g_global_pool->tls_key);
    if (!pool) {
        ...
        // Initialize the pool with default values
        if (!memory_pool_init(pool, DEFAULT_BLOCK_SIZE, DEFAULT_MAX_BLOCKS, DEFAULT_SMALL_CAPACITY)) {
            free(pool);
            return NULL;
        }
...
void tls_pool_get_stats(size_t* total_allocated, size_t* max_allocated,
    ...
    if (!pool) {
        ...
        if (num_allocs) *num_allocs = 0;
        if (num_frees) *num_frees = 0;
        if (cache_misses) *cache_misses = 0;
        return;
    }
    
    if (total_allocated) *total_allocated = pool->total_allocated;
    if (max_allocated) *max_allocated = pool->max_allocated;
    if (num_allocs) *num_allocs = pool->num_allocs;
    if (num_frees) *num_frees = pool->num_frees;
