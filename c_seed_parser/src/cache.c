...
cache_t* cache_create(size_t capacity, size_t num_buckets, cache_policy_t policy, 
    ...
    cache->buckets = (cache_entry_t**)calloc(num_buckets, sizeof(cache_entry_t*));
    if (!cache->buckets) {
        free(cache);
        return NULL;
    }
...
bool cache_put(cache_t* cache, const void* key, size_t key_len, const void* value, size_t value_size) {
    ...
    if (cache->prune_interval > 0 && now - cache->last_prune >= cache->prune_interval) {
        cache_prune(cache, 0);
    ...
    if (cache->size + value_size > cache->capacity) {
        ...
        size_t pruned = cache_prune(cache, target_size);
...
void cache_get_stats(cache_t* cache, cache_stats_t* stats) {
    ...
    stats->hit_rate = total_lookups > 0 ? (double)cache->hits / total_lookups : 0.0;
    
    // Calculate average lookup time
    stats->avg_lookup_time = cache->num_lookups > 0 ? 
        cache->total_lookup_time / cache->num_lookups : 0.0;
    
    // Calculate average insert time
    stats->avg_insert_time = cache->num_inserts > 0 ? 
        cache->total_insert_time / cache->num_inserts : 0.0;
}
...
bool cache_resize(cache_t* cache, size_t new_capacity, size_t new_num_buckets) {
    ...
    cache_entry_t** new_buckets = (cache_entry_t**)calloc(new_num_buckets, sizeof(cache_entry_t*));
    if (!new_buckets) {
        return false;
    }
...
void cache_for_each(cache_t* cache, void (*callback)(const void* key, size_t key_len, 
    ...
    for (size_t i = 0; i < cache->num_buckets; i++) {
        ...
        while (entry) {
            ...
            callback(&key_hash, sizeof(key_hash), entry->value, entry->value_size, user_data);
