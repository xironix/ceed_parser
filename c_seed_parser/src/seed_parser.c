...
/**
 * @brief Default number of maximum repeated words allowed
 */
#define DEFAULT_EXWORDS 2
...
static DBController* db_init(const SeedParserConfig *config) {
    ...
    db->in_memory = (config->db_path == NULL || strcmp(config->db_path, ":memory:") == 0);
    ...
    /* Create tables */
    const char *create_phrases_table =
    ...
    if (sqlite3_exec(db->db, create_phrases_table, NULL, NULL, NULL) != SQLITE_OK) {
    ...
    /* Create index */
    const char *create_index = "CREATE INDEX IF NOT EXISTS idx_phrases_timestamp ON phrases(timestamp)";
 ...
 */
static int db_add_phrase(DBController *db, const char *phrase, MnemonicType type, MnemonicLanguage language) {
    ...
    if (db->batch_count >= db->batch_size) {
        ...
        sqlite3_stmt *stmt;
        const char *sql = "INSERT OR IGNORE INTO phrases (phrase, type, language, timestamp) VALUES (?, ?, ?, ?)";
 ...
 */
static void process_mnemonic(SeedParser *parser, const char *mnemonic, const char *source_file) {
    MnemonicType type;
 ...
 */
static bool valid_phrase_repetition(const char **words, size_t count, int max_repetition) {
    if (max_repetition <= 0) {
 ...
 */
static void process_word_window(SeedParser *parser, char **word_window, size_t window_size,
    ...
    for (size_t i = 0; chain_sizes[i] != 0; i++) {
        ...
        for (size_t start = 0; start <= window_size - size; start++) {
            ...
            if (!valid_phrase_repetition((const char **)(word_window + start), size, parser->config->exwords)) {
                continue;
            }
...
static int process_file(SeedParser *parser, const char *filepath) {
    ...
    if (should_skip_extension(filepath) || should_skip_file(basename(filepath))) {
        return 0;
    }
    ...
    size_t bytes_read;
    while ((bytes_read = fread(buffer, 1, parser->config->chunk_size, file)) > 0) {
            ...
            if (words && word_count > 0) {
                ...
                for (size_t i = 0; i < word_count; i++) {
                    ...
                    if (window_size >= MAX_WINDOW_SIZE) {
                        free(word_window[0]);
                        memmove(word_window, word_window + 1, (MAX_WINDOW_SIZE - 1) * sizeof(char*));
                        window_size--;
                    }
...
static int open_log_files(SeedParser *parser) {
    ...
    /* Build paths */
    snprintf(seed_path, sizeof(seed_path), "%s/seed-%s.txt", parser->config->log_dir, timestamp);
    snprintf(addr_path, sizeof(addr_path), "%s/addresses-%s.txt", parser->config->log_dir, timestamp);
    snprintf(full_path, sizeof(full_path), "%s/full-log-%s.txt", parser->config->log_dir, timestamp);
    snprintf(eth_addr_path, sizeof(eth_addr_path), "%s/eth-a-log-%s.txt", parser->config->log_dir, timestamp);
    snprintf(eth_key_path, sizeof(eth_key_path), "%s/eth-p-log-%s.txt", parser->config->log_dir, timestamp);
    snprintf(monero_path, sizeof(monero_path), "%s/monero-log-%s.txt", parser->config->log_dir, timestamp);
...
int seed_parser_config_init(SeedParserConfig *config) {
    ...
    config->exwords = DEFAULT_EXWORDS;
...
int seed_parser_init(const SeedParserConfig *config) {
    ...
    g_parser.threads = (pthread_t*)malloc(g_parser.num_threads * sizeof(pthread_t));
    if (!g_parser.threads) {
        seed_parser_cleanup();
        return -1;
    }
    ...
    for (size_t i = 0; i < LANGUAGE_COUNT; i++) {
        ...
        if (mnemonic_load_wordlist(g_parser.mnemonic_ctx, config->languages[i]) != 0) {
...
int seed_parser_start(void) {
    ...
    for (int i = 0; i < g_parser.num_threads; i++) {
        if (pthread_create(&g_parser.threads[i], NULL, worker_thread, &g_parser) != 0) {
